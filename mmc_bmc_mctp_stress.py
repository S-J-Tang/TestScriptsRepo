...
def run_test_cycle(ip, cycle_num, logger):
    ...
    if not ssh_util.run_command(ssh, "busctl call au.com.codeconstruct.MCTP1 /au/com/codeconstruct/mctp1/interfaces/mctpi3c0 au.com.codeconstruct.MCTP.BusOwner1 LearnEndpoint ay 6 0x06 0x32 0x12 0x34 0x55 0x55", logger):  # If any command fails, return False
        ...
        return False
    time.sleep(10)
    if not ssh_util.run_command(ssh, "busctl call au.com.codeconstruct.MCTP1 /au/com/codeconstruct/mctp1/interfaces/mctpi3c0 au.com.codeconstruct.MCTP.BusOwner1 LearnEndpoint ay 6 0x06 0x32 0x12 0x34 0x55 0x67", logger):  # If any command fails, return False
        logger.error(f"Error detected in learning MCTP endpoint during cycle {cycle_num}. Rebooting BMC...")
        ssh.exec_command("reboot")
        time.sleep(5)
        if not bbu.check_bmc_reboot(ip, logger):
    ...
    logger.info("Getting TID using pldmtool...")
    if not ssh_util.run_command(ssh, "pldmtool base GetTID -m 20", logger):  # If any command fails, return False
        logger.error(f"Error detected in TID retrieval during cycle {cycle_num}. Rebooting BMC...")
        ssh.exec_command("reboot")
        time.sleep(5)
        if not bbu.check_bmc_reboot(ip, logger):
    ...
    logger.info("Getting Firmware parameters using pldmtool...")
    if not ssh_util.run_command(ssh, "pldmtool fw_update GetFwParams -m 20", logger):  # If any command fails, return False
        logger.error(f"Error detected in firmware parameter retrieval during cycle {cycle_num}. Rebooting BMC...")
        ssh.exec_command("reboot")
        time.sleep(5)
        if not bbu.check_bmc_reboot(ip, logger):
